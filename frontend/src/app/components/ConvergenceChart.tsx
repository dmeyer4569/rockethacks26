...
export function ConvergenceChart() {
  ...
  const threshold = activeSimulation?.config.convergence_threshold ?? 0.60;
  ...
  const { data, agents } = useMemo(() => {
    ...
    for (const round of rounds) {
      for (const p of round.proposals) {
        agentNames.add(p.persona_name);
      }
    }
    const agentList = Array.from(agentNames);
    const agentsMeta = agentList.map((name, i) => ({
      key: name,
      label: name,
    ...
    const chartData = rounds.map((round) => {
      ...
      for (const p of round.proposals) {
        point[p.persona_name] = p.cas;
      }
      return point;
    });

    return { data: chartData, agents: agentsMeta };
  }, [rounds]);
  ...
  const handleMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    const stepW = data.length > 1 ? plotW / (data.length - 1) : plotW;
    let closestIdx = data.length > 1
      ? Math.round((mouseX - CHART_PADDING.left) / stepW)
      : 0;
    closestIdx = Math.max(0, Math.min(data.length - 1, closestIdx));

    const d = data[closestIdx];
    const x = CHART_PADDING.left + (data.length > 1 ? (closestIdx / (data.length - 1)) * plotW : plotW / 2);

    setTooltip({
      x,
      y: CHART_PADDING.top,
      round: `R${closestIdx + 1}`,
      values: agents.map((a) => ({
        label: a.label,
        value: d[a.key] ?? 0,
        color: a.color,
        ...
        {isEmpty ? (
          ...
          <>
            ...
            >
              {Y_TICKS.map((tick) => {
                ...
                return (
                  <g key={`grid-${tick}`}>
                    ...
                    <text x={CHART_PADDING.left - 8} y={y + 4} textAnchor="end" fill="#64748b" fontSize={11} fontFamily="Roboto Mono, monospace">
                      {tick.toFixed(1)}
                    </text>
                  </g>
                );
              ...
              <text x={14} y={CHART_PADDING.top + plotH / 2} textAnchor="middle" fill="#475569" fontSize={11} fontFamily="Inter, sans-serif" transform={`rotate(-90, 14, ${CHART_PADDING.top + plotH / 2})`}>
                CAS Score
              </text>

              {data.map((_, i) => {
                const x = CHART_PADDING.left + (data.length > 1 ? (i / (data.length - 1)) * plotW : plotW / 2);
                return (
                  <text key={`xlabel-${i}`} x={x} y={chartH - 6} textAnchor="middle" fill="#64748b" fontSize={11} fontFamily="Roboto Mono, monospace">
                    R{i + 1}
                  </text>
                );
              })}
              ...
              {data.map((_, i) => {
                const x = CHART_PADDING.left + (data.length > 1 ? (i / (data.length - 1)) * plotW : plotW / 2);
                return (
                  <line key={`vgrid-${i}`} x1={x} y1={CHART_PADDING.top} x2={x} y2={CHART_PADDING.top + plotH} stroke="rgba(255,255,255,0.03)" strokeDasharray="3 3" />
                );
              })}

              <line x1={CHART_PADDING.left} y1={thresholdY} x2={CHART_PADDING.left + plotW} y2={thresholdY} stroke="#34d399" strokeDasharray="6 4" strokeWidth={1.5} />
              <text x={CHART_PADDING.left + plotW - 4} y={thresholdY - 6} textAnchor="end" fill="#34d399" fontSize={10} fontFamily="Roboto Mono, monospace">
                Consensus: {threshold.toFixed(2)}
              </text>

              {agents.map((agent) => (
                <polyline key={`line-${agent.key}`} points={buildPolyline(data, agent.key, chartW, chartH)} fill="none" stroke={agent.color} strokeWidth={2} strokeLinejoin="round" strokeLinecap="round" />
              ))}

              {agents.map((agent) =>
                getDotPositions(data, agent.key, chartW, chartH).map((pos, i) => (
                  <circle key={`dot-${agent.key}-${i}`} cx={pos.x} cy={pos.y} r={3} fill={agent.color} />
                ))
              )}

              {tooltip && (
                <line x1={tooltip.x} y1={CHART_PADDING.top} x2={tooltip.x} y2={CHART_PADDING.top + plotH} stroke="rgba(255,255,255,0.15)" strokeWidth={1} />
              )}
            </svg>
            ...
            {tooltip && (
              ...
              >
                <div className="bg-[#0B1120] border border-white/[0.1] rounded-lg px-3 py-2 shadow-xl">
                  <p className="font-['Roboto_Mono',monospace] text-slate-400 mb-1" style={{ fontSize: "11px" }}>
                    {tooltip.round}
                  </p>
                  {tooltip.values.map((v) => (
                    <div key={v.label} className="flex items-center gap-2">
                      <div className="w-2 h-2 rounded-full" style={{ backgroundColor: v.color }} />
                      <span className="font-['Roboto_Mono',monospace] text-slate-300 whitespace-nowrap" style={{ fontSize: "11px" }}>
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-3 pt-3 border-t border-white/[0.04]">
          {agents.map((agent) => (
            <div key={agent.key} className="flex items-center gap-1.5">
              ...
              <span className="font-['Inter',sans-serif] text-slate-400" style={{ fontSize: "11px" }}>
                {agent.label}
              </span>
            </div>
          ))}
