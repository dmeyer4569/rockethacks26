...
async def generate_proposal(persona: dict, current_policy: str, case: dict, domain: str = "Policy") -> str:
    ...
    user_prompt = (
        ...
        f"- Your changes MUST directly modify the existing policy text below. Do NOT introduce unrelated mechanisms.\n"
        f"- Stay strictly within the '{domain}' domain. If the policy is about healthcare, propose healthcare changes — not tax credits, not tech subsidies, etc.\n"
        f"- If your usual priorities don't neatly apply to this policy, find the closest relevant angle within the policy's actual scope.\n\n"
        f"COURSE-OF-ACTION:\n{current_policy}\n\n"
...
async def rate_proposal(persona: dict, current_policy: str, proposal: dict, domain: str = "Policy") -> str:
    ...
    user_prompt = (
        ...
        f"RATING GUIDELINES:\n"
        f"- Weigh BOTH your personal interests AND the proposal's broader merit for the '{domain}' domain.\n"
        f"- A proposal that is broadly beneficial but neutral to you personally should score at least +0.10 to +0.30.\n"
        f"- Only use extreme scores (-0.80 to -1.00 or +0.80 to +1.00) for proposals with genuinely extreme impact.\n"
        f"- If the proposal is off-topic or introduces unrelated mechanisms, rate it near 0.00 regardless of whether it benefits you.\n\n"
